...
添加「耗材类型」条目后，点击条目进入配置界面，菜单包含：**新建分组 → 修改分组 → 删除分组 → 设置阈值 → 通知渠道设置**。监控以**分组**为单位——每个分组独立生成诊断实体与数据传感器，可各自覆盖条目级阈值。

#### 新建分组（两种类别）

**① 绑定实体分组**（关联传感器 / 群组，按实体值评估）：
- **实体多选**：直接勾选要监控的实体（如 `sensor.air_purifier_filter_life`）。
- **正则批量匹配**：输入正则表达式，运行时动态匹配当前实体 ID（已禁用实体自动排除），例如 `sensor\..*filter.*`；**新增匹配实体自动入组，无需重新配置**。
- **群组绑定**：可选择辅助元素中的**群组**（`group.xxx`），每次刷新自动把群组成员展开为实际实体逐个评估，群组增减成员自动跟随。
- 手动多选与正则匹配提交时取**并集**。
- 可勾选**覆盖条目级阈值**，为当前分组单独设定阈值类型 / 数值 / 单位 / 计算方式。

**② 自定义耗材实体**（自建倒计时数据，无需绑定任何实体）：
- 填写名称、可选绑定耗材、**添加/更换时间**与**建议寿命**（天 / 小时 / 分钟），自动生成倒计时数据实体。
- 同样可覆盖条目级阈值，按已使用时长判定「需要更换」。
...
> 阈值默认值三级兜底：条目已存配置 → 库类型元数据（内置库或自定义类型向导中的默认阈值）→ 通用兜底（剩余寿命 20%）。分组覆盖阈值时以分组为准。
...
| **型号** | 仅自定义方式填写（英文 / 数字），参与耗材 ID 生成与用户库去重。 |
| **单位** | 下拉选择标准单位（个 / 片 / 张 / 瓶…），自动翻译显示。 |
...
### 耗材类型条目
每个**分组**生成一对实体（诊断 + 数据），自定义耗材实体分组另有倒计时实体：
| 实体 | 说明 |
| :--- | :--- |
| `sensor.{分组名} 更换状态` | **状态**：`正常` / `需要更换`（枚举，诊断类，每组 1 个）。**属性**：耗材类型、分组、触发实体、阈值（类型/值/单位/计算方式）、上次更换时间、绑定耗材信息。 |
| `sensor.{分组名}数据` | **状态**：组内监控值的最小值（纯数值，不带单位，便于自动化比较）。**属性**：分组、耗材类型、各绑定实体实时值（含绑定耗材）。 |
| `sensor.{自定义名称}（倒计时）` | 仅自定义耗材实体分组——**状态**：剩余寿命（按寿命单位 d / h / min，`device_class: duration`）。**属性**：建议寿命、添加/更换时间。 |
| `todo.xx 待办事项` | 更换待办：任一分组越过阈值自动生成「更换 XX」，**勾选完成 = 已更换**（记录更换时间 + 自动扣减关联库存）。 |
...
### consumable_manager.query_data — 查询数据
提取本集成的结构化数据（JSON，**仅响应服务**，配合 `response_variable` 使用），适合接入仪表板或外部系统。必须指定 `data_type`：
| data_type | 返回内容 |
| :--- | :--- |
| `stock` | 库存条目（可按 item 过滤） |
| `type_entry` | 耗材类型条目状态 |
| `group_data` | 分组实体数据（成员含绑定耗材字段） |
| `types` | 耗材类型元数据 |
| `consumables` | 全部耗材（可按 type 过滤） |
...
将设备耗材实体绑定到耗材库中的耗材（**纯元数据映射**，写入独立绑定层，仅用于待办 / 通知显示规格，不触发监控）。手动指定 `consumable_id`，或只传关联库存项 `item`（自动继承该库存项关联的耗材）。
```yaml
service: consumable_manager.bind_entity
data:
  entity_id: sensor.air_purifier_filter_life
  consumable_id: filter_hepa13   # 手输耗材 ID（文本）
  # item: sensor.study_filter    # 或传库存项实体，继承其关联耗材
```

### consumable_manager.unbind_entity — 解绑实体
删除实体与耗材的绑定映射（仅删独立绑定层，不影响条目的监控配置）。
```yaml
...
查询实体与耗材的绑定关系（按实体、耗材或库存项过滤），响应含各实体的越阈状态（`triggered`）。

### consumable_manager.add_consumable — 添加耗材到用户库
按模板字段添加耗材，写入本地用户库。耗材 ID 自动生成（类型 + 型号），如 `filter_hepa13`。`model` 与 `unit` 必须为英文（ASCII；单位为 18 个标准键之一，如 `piece` / `filter`），配置界面同字段为下拉选择。
```yaml
...
### 双库架构与绑定层
- **内置库**（`custom_components/consumable_manager/library/`）：随集成分发——`index.json`（类型元数据）、`consumables.json`（耗材）、`names.json`（多语言映射）。
- **用户库**（`config/.consumable_manager/user_library.json`）：本地数据，配置界面与服务写入，也支持手工编辑（重启生效）。同锚点条目用户优先、整条替换；坏文件自动降级忽略并告警。
- **绑定层**（HA 存储 `.storage/consumable_manager.bindings`）：实体 ↔ 耗材的绑定映射，独立于条目配置持久化，仅供待办 / 通知显示规格；由「绑定实体 / 解绑实体」服务维护。
...
2. PR 到 **main** 分支。
3. GitHub Actions 自动运行摄入工具：字段校验（代码 / 标识字段强制 ASCII）、锚点去重、多语言拆解、排序归位，组装进内置库并回推 PR（草稿自动移除）。
4. 维护者审核后合入主分支。

详见 [library/README.md](custom_components/consumable_manager/library/README.md)（中英双语，随库分发）。
